#!/usr/bin/env bash
# rollback_to_v165.sh — jianji 回退到稳定版 v1.6.5 的防御脚本
#
# 适用场景：v1.6.6 发布后若发现严重问题，用它快速发一个「更高 versionCode 的 v1.6.5」，
#           覆盖安装即可把 App 行为回退到稳定版（CI 自动构建签名 APK 并建 Release）。
#
# 触发方式：GitHub Actions（build-apk.yml）在打 tag 时自动构建，因此本脚本只负责
#           基于 v1.6.5 的源码树创建一次专门的高 versionCode 提交 + annotated tag，
#           由 tag push 触发 CI。
#
# ⚠️ 数据风险提示（务必先看）：
#   1) v1.6.6 把数据库从 v5 升到 v6（金额改分存储、新增标签/回收站）。
#      回退到 v1.6.5（DB v5）属于「降 schema」，Room 不允许自动降级，
#      覆盖安装后首次打开可能报 IllegalStateException（降级被拒）。
#   2) 因此回退后，若 App 起不来，请在系统设置里「清除应用数据」再打开；
#      这意味着 v1.6.6 的专属数据（标签、软删记录）回退即弃，属于预期内损失。
#   3) v1.6.5 之前的核心交易/分类数据如果仍在 v6 schema 中，回退后会被丢弃——
#      所以本脚本默认「仅在确认不需要 v1.6.6 数据时使用」。
#
# 用法:
#   GITHUB_TOKEN=<token> ./rollback_to_v165.sh
#   （或用已 gh 登录的环境直接执行）
#
...
BASE_TAG="v1.6.5"
ROLLBACK_TAG="rollback/v1.6.5"
ROLLBACK_VC=40            # 必须 > v1.6.6 的 39，否则无法覆盖安装
ROLLBACK_VN="1.6.5"

die(){ echo "::error:: $*" >&2; exit 1; }

# 1) 解析 v1.6.5 annotated tag -> commit sha
...
# 3) 构造新的 app/build.gradle.kts：versionCode=40 / versionName="1.6.5"
...
# 6) 建 commit（基于 v1.6.5 的 parent）
parent_info=$(python3 - "$commit_sha" "$new_tree_sha" "$ROLLBACK_TAG" <<'PY'
import json,sys
p,t,tag=sys.argv[1],sys.argv[2],sys.argv[3]
print(json.dumps({"message":f"rollback: rebuild {tag} (vc={40}) on top of v1.6.5",
  "tree":t,"parents":[p]}))
PY
)
