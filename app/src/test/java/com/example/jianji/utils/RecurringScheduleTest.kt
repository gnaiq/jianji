...
class RecurringScheduleTest {
    @Test fun monthlyClampsDay31ToMonthLength() {
        ...
        val r = computeRecurringNextRun(RecurringFrequency.MONTHLY, 31, 1, 1, monthOfYear = 1, now = now)
    ...
    @Test fun monthlyPastDaySkipsToNextMonth() {
        ...
        val now = LocalDateTime.of(2026, 4, 15, 12, 0)
        val r = computeRecurringNextRun(RecurringFrequency.MONTHLY, 5, 1, 1, monthOfYear = 1, now = now)
    ...
    @Test fun weeklyHonorsDayOfWeek() {
        ...
        val r = computeRecurringNextRun(RecurringFrequency.WEEKLY, 1, 1, 1, monthOfYear = 1, now = now)
        assertEquals(expected.atStartOfDay(), r)
    }

    @Test fun dailyAdvancesOneDay() {
        val now = LocalDateTime.of(2026, 4, 15, 12, 0)
        val r = computeRecurringNextRun(RecurringFrequency.DAILY, 1, 1, 1, monthOfYear = 1, now = now)
