#!/usr/bin/env bash
# v1.6.4 回退到 v1.6.3 脚本（默认不执行）。
#
# 背景：v1.6.4 未升级 DB schema（仍为 v5），与 v1.6.3 数据完全兼容。
# 因此回退有两种方式：
#   方式A（推荐，零构建）：设备「卸载 v1.6.4」→ 重装已发布的 v1.6.3 APK，
#        公共下载目录的自动备份 JSON 仍在，数据无损。
#   方式B（本脚本，覆盖安装免卸载）：重建 v1.6.3 源码为新 tag v1.6.3.1
#        （versionCode=38 > v1.6.4 的 37，可覆盖安装），走 CI 重新出包。
#
# 本脚本采用方式B：以 v1.6.3 发布树的源码为基，仅把 versionCode/versionName
# 提到 38 / 1.6.3.1，满足 CI 的 versionCode 单调递增 + versionName==tag 守卫，
# 让 v1.6.3.1 能直接「升级覆盖」已装的 v1.6.4。
#
# 纪律：Git Data API 通路；annotated tag 两步走。
set -euo pipefail
cd "${WORKDIR:-/tmp/jianji-v163}"
REPO=gnaiq/jianji
SRC_TAG=v1.6.3
NEW_TAG=v1.6.3.1
MSG="rollback: 回退到 v1.6.3（tag v1.6.3.1, versionCode=38）——覆盖安装免卸载"

if gh api "repos/$REPO/releases/tags/$NEW_TAG" >/dev/null 2>&1; then
  echo "ERROR: $NEW_TAG 已有 Release，禁止重发"; exit 1
fi

# v1.6.3 标注 tag -> tag 对象 -> commit -> tree
...
echo "v1.6.3 source tree = $BASE_TREE"

# 取 v1.6.3 的 build.gradle.kts，改 versionCode/versionName
...
NEW_GRADLE=$(printf '%s' "$RAW" \
  | sed -E 's/versionCode = [0-9]+/versionCode = 38/' \
  | sed -E 's/versionName = "[^"]+"/versionName = "1.6.3.1"/')
...
echo "DONE rollback tag=$NEW_TAG commit=$COMMIT_SHA —— CI 将构建 v1.6.3.1 APK（覆盖安装免卸载）"
